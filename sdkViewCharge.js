...
function ViewCharge(after, before, status) {
  ...
  do {
    query["cursor"] = cursor;
    json = JSON.parse(fetch("/charge", method = 'GET', null, query).content);
    elementList = json["charges"];
    cursor = json["cursor"];
    ...
    for (let charge of elementList){
      ...
      sheet.getRange('A' + i.toString()).setValue(formatToLocalDatetime(charge["issueDate"]));
      ...
      sheet.getRange('F' + i.toString()).setValue(formatToLocalDatetime(charge["dueDate"]));
